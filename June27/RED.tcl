#create Simulator object
set ns [new Simulator]

set f1 [open redTail.tr w]
$ns trace-all $f1

set f2 [open redTail.nam w]
$ns namtrace-all $f2

#proc finish{} {puts "complete"; exit 0 }
proc finish {} { puts "complete"; exit 0 }


#creating nodes
set n0 [$ns node]
set n1 [$ns node]
set n2 [$ns node]
set n3 [$ns node]
set n4 [$ns node]
set n5 [$ns node]
set n6 [$ns node]
set n7 [$ns node]

#create link. Here we use duplex link

#Set link b/n node0 & node3
$ns duplex-link $n0 $n3 20Mb 10ms RED
$ns duplex-link $n1 $n3 20Mb 10ms RED
$ns duplex-link $n2 $n3 20Mb 10ms RED
#Set link b/n node3 & node4
$ns duplex-link $n3 $n4 1Mb 10ms RED 
#Set link b/n node4 & node5
$ns duplex-link $n4 $n5 20Mb 10ms RED 
$ns duplex-link $n4 $n6 20Mb 10ms RED
$ns duplex-link $n4 $n7 20Mb 10ms RED

#Creating the protocol agents of source
#set udp0 [ new Agent/UDP] #proto agents ovewr here are UDP

#Create proto Agent to the source
#Attach agent to source
set udp0 [new Agent/UDP]
$ns attach-agent $n0 $udp0 

set udp1 [new Agent/UDP]
$ns attach-agent $n1 $udp1

set udp2 [new Agent/UDP]
$ns attach-agent $n2 $udp2
#all source agents created

#creating proto agents of Dest
#udp dest is set as Null. creating dest agents
#Dest agent is udp and it is null
set null5 [new Agent/Null] 
$ns attach-agent $n5 $null5


set null6 [new Agent/Null]
$ns attach-agent $n6 $null6


set null7 [new Agent/Null]
$ns attach-agent $n7 $null7

#all Dest agents created

#Creating the user applications. Here the user app is CBR
set cbr0 [new Application/Traffic/CBR]
$cbr0 attach-agent $udp0
$cbr0 set packetSize_ 1000
$cbr0 set interval_ .005

set cbr1 [new Application/Traffic/CBR]
$cbr1 attach-agent $udp1
$cbr1 set packetSize_ 1000
$cbr1 set interval_ .005

set cbr2 [new Application/Traffic/CBR]
$cbr2 attach-agent $udp2
$cbr2 set packetSize_ 1000
$cbr2 set interval_ .005

#Connecting different source and Destinations

#Connect n0 to n5
$ns connect $udp0 $null5  
#Connect n1 to n6
$ns connect $udp1 $null6  
#Connect n2 to n7
$ns connect $udp2 $null7 

#Set flowid values since we are using UDP
$udp0 set fid_ 1
$udp1 set fid_ 2
$udp2 set fid_ 3

#start the applications
#App objects are $vars eg. $cbr0 
$ns at 0.1 "$cbr0 start"
$ns at 10.0 "$cbr0 stop"

$ns at 0.1 "$cbr1 start"
$ns at 10.0 "$cbr1 stop"

$ns at 0.1 "$cbr2 start"
$ns at 10.0 "$cbr2 stop"

$ns at 10.5 "finish"

$ns run
#to run 
#ns queuedrop.tcl

#nam quueudrop.nam

